...
const MortgageRentDetailAdvertisement: FC<IProps> = ({ housesList }) => {
  return (
    <div className='flex flex-col gap-6 mt-12'>
      <div className='flex flex-row justify-between h-[42px] rounded-[12px] dark:bg-[#393939] bg-white dark:shadow-none shadow-[0_0px_16px_rgba(0,0,0,0.27)]'>
        ...
        <Link href={'/mortgage-rent'} className='flex flex-row gap-2 text-[#66b436] dark:text-[#8CFF45] items-center ml-2'>
          <div>مشاهده همه</div>
        ...
        {housesList.houses.map((houses, index) => {
          return (
            <SwiperSlide key={index}>
              <AdvertisementList houseListDetail={houses} />
            </SwiperSlide>
          )
        })}
      </Swiper>
...
export default MortgageRentDetailAdvertisement