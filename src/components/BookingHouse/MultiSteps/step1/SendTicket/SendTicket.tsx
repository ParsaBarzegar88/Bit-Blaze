...
const SendTicket = () => {
    ...
    useEffect(() => {
        ...
        if (bookCookie) {
            ...
            if (parsedCookie.shareMobile) setShareMobile(parsedCookie.shareMobile);
            if (parsedCookie.shareEmail) setShareEmail(parsedCookie.shareEmail);
        }
    }, [cookie]);
    useEffect(() => {
        const bookCookie = cookie.get('book');
        const bookData = bookCookie ? JSON.parse(bookCookie) : {};
        bookData.shareMobile = shareMobile;
        bookData.shareEmail = shareEmail;
            ...
            <div className='w-full flex flex-col sm:flex-row justify-between items-center mb-4 p-2 rounded-xl dark:shadow-none shadow-md border dark:border-0 dark:border-[#333] border-gray-200 dark:bg-[#4D4D4D] bg-gray-100'>
                <div className='flex flex-row gap-2 items-center text-lg dark:text-white text-gray-800 mb-2 sm:mb-0'>
                    ...
                    <span className='flex flex-row items-center gap-1'>
                        ارسال بلیط به دیگری
                        <div className='text-[#4f9623] dark:text-[#8CFF45] text-[13px]'>(ارسال بلیط به ایمیل و شماره همراه دیگر)</div>
                    </span>
                </div>
            </div>
